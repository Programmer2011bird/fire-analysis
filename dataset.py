...
def get_dataloader():
    ...
    province_to_idx = {province: idx for idx, province in enumerate(unique_provinces) }
    
    df["province"] = df["province"].map(province_to_idx)
    
    X = torch.tensor(df["province"].values)
    Y = torch.tensor(df["frp"].values)
    
    class fire_dataset(Dataset):
        def __init__(self, province, frp):
        ...
        def __getitem__(self, index):
            return self.province[index], self.frp[index]
